...
<head>
   ...
   <title>Siparişi görüntüle</title>
...
<section class="order-details">
   ...
   <h1 class="heading">Sipariş Bilgileri</h1>
   ...
   <div class="box">
      <div class="col">
         ...
         <p class="price">$</i> <?= $fetch_order['price']; ?> x <?= $fetch_order['qty']; ?></p>
         <h3 class="name"><?= $fetch_product['name']; ?></h3>
         <p class="grand-total">Toplam Tutar : <span>$ <?= $grand_total; ?></span></p>
      </div>
      <div class="col">
         <p class="title">Fatura Bilgisi</p>
         ...
         <?php if($fetch_order['status'] == 'İptal Edildi'){ ?>
            <a href="odeme.php?get_id=<?= $fetch_product['id']; ?>" class="btn">order again</a>
         <?php }else{ ?>
         <form action="" method="POST">
            <input type="submit" value="Siparişi iptal et" name="cancel" class="delete-btn" onclick="return confirm('İptal edilsin mi?');">
         </form>
         <?php } ?>
      </div>
   </div>
